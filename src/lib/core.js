...
function buildRemoteInstallCommand() {
  return [
    ...
    'sudo -n systemd-run --unit="$unit" --collect --no-block --property=Type=exec --property=TimeoutStartSec=infinity --property=Nice=10 --property="StandardOutput=append:$log" --property="StandardError=append:$log" /usr/bin/env "PATH=$throttle_path" /bin/bash "$script" >/dev/null',
    'state=""',
