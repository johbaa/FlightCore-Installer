...
function buildRemoteInstallCommand() {
  return [
    ...
    '# test.10 physical evidence proved PATH interception was bypassed and multiple cc1plus',
    '# processes still ran concurrently until swap was exhausted. Enforce serialization inside the',
    '# detached service mount namespace so both relative and absolute /usr/bin/ninja calls see -j1.',
    '# test.11 proved that a wrapper sourced from /run is not a valid executable source for Meson/Ninja detection on this Pi.',
    '# Keep both shim files on the normal executable root filesystem under the installer-owned /var/lib directory.',
    'sudo -n install -m 755 /usr/bin/ninja "$ninja_real"',
    'sudo -n chmod 755 "$ninja_real"',
    `printf '%s\\n' '#!/bin/sh' 'if [ "\${1:-}" = "--version" ]; then exec /var/lib/flightcore-native-installer/ninja-real --version; fi' 'exec /var/lib/flightcore-native-installer/ninja-real -j1 "$@"' | sudo -n tee "$ninja_wrapper" >/dev/null`,
    'sudo -n chmod 755 "$ninja_wrapper"',
    ...
    '# Keep native compilation genuinely single-job and suspend the hardware watchdog only inside the transaction.',
    '# BindReadOnlyPaths applies inside the transient unit mount namespace and cannot be bypassed by sudo secure_path.',
    'sudo -n systemd-run --unit="$unit" --collect --no-block --property=Type=exec --property=TimeoutStartSec=infinity --property=Nice=10 --property="BindReadOnlyPaths=$ninja_wrapper:/usr/bin/ninja" --property="StandardOutput=append:$log" --property="StandardError=append:$log" /bin/bash "$guard" >/dev/null',
    'state=""',
