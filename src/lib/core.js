...
function buildRemoteInstallCommand() {
  return [
    ...
    "ninja_real='/run/flightcore-native-installer-ninja-real'",
    "ninja_wrapper='/run/flightcore-native-installer-ninja'",
    ...
    '# RC11/test.10 physical evidence proved PATH interception was bypassed and multiple cc1plus',
    ...
    'sudo -n cp /usr/bin/ninja "$ninja_real"',
    'sudo -n chmod 700 "$ninja_real"',
    `printf '%s\\n' '#!/bin/sh' 'exec /run/flightcore-native-installer-ninja-real -j1 "$@"' | sudo -n tee "$ninja_wrapper" >/dev/null`,
    'sudo -n chmod 700 "$ninja_wrapper"',
